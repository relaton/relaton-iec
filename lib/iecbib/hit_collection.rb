...
module Iecbib
  ...
  class HitCollection < Array
    ...
    def initialize(ref_nbr, year = nil) #(text, hit_pages = nil)
      ...
      url = "#{DOMAIN}/searchkey&RefNbr=#{ref_nbr}&From=#{from.to_s}&To=#{to.to_s}&start=1"
      doc = Nokogiri::HTML OpenURI.open_uri(url)
      hits =  doc.css('ul.search-results > li').map do |h|
        code = h.at('a[2]').text.tr [194, 160].pack('c*').force_encoding('UTF-8'), ''
        title = h.children[5].text
        url = DOMAIN + h.at('a[2]')[:href]
